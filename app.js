...
function dispaly(data) {
    data.map(employees => {
        ...
        downloadBtn.addEventListener("click", (e) => {
            const id = e.target.getAttribute("data-id");
            createPDF( employees); // Pass the employee data for download
        });
        ...
        function createPDF(employee) {
            ...
            const { id, name, email, salary, department, phoneNumber, address } = employee;

            // Create a new jsPDF instance (use the proper method for jsPDF v2.x)
            ...
            // Add text to the PDF
            doc.text(`Employee ID: ${id}`, 10, 10);
            doc.text(`Name: ${name}`, 10, 20);
            doc.text(`Email: ${email || "N/A"}`, 10, 30);
            doc.text(`Salary: ${salary || "N/A"}`, 10, 40);
            doc.text(`Department: ${department || "N/A"}`, 10, 50);
            doc.text(`Phone: ${phoneNumber || "N/A"}`, 10, 60);
            doc.text(`Address: ${address || "N/A"}`, 10, 70);

            // Save the PDF
            doc.save(`employee_${id}.pdf`);
        }
